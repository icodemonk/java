...
public class TruckService {
    ...
    {
        String sql="insert into trucks (vehical_number,vehical_model,vehical_capacity,drivername) values (?,?,?,?)";
        try
        {
    ...
    public Truck getTruckById(int id)
    {
        ...
        try
        {
            ...
            if(resultSet.next())
            {
                ...
                truck.setName(resultSet.getString("vehical_number"));
                truck.setModel(resultSet.getString("vehical_model"));
                truck.setCapacity(resultSet.getString("vehical_capacity"));
    ...
    {
        String sql="update trucks set vehical_number = ?, vehical_model = ?, vehical_capacity = ?, drivername = ? where id= ?";
        try{
            Connection connection = ConnectionDetails.getConnection();
            PreparedStatement preparedStatement = connection.prepareStatement(sql);
            preparedStatement.setString(1,truck.getName());
